...
fn main() {
    ...
    // Vendored patch: on Windows MSVC, bindgen regeneration emits an opaque
    // `whisper_full_params` (known libclang issue with anonymous nested structs
    // + function-pointer fields), so always use the bundled bindings. The
    // bundled file is patched to neutralize Linux glibc layout asserts that
    // would otherwise fail on Windows.
    let force_bundled = cfg!(target_os = "windows")
        || env::var("WHISPER_DONT_GENERATE_BINDINGS").is_ok();
    if force_bundled {
